...
public class ShopTest {
	...
	public void searchSangpum() {
		...
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		System.out.println("검색할 상품명?(일부도 가능)");
		String name=sc.nextLine();
		
		String sql="select num,type,sangpum,su,dan,ipgoday from sales where sangpum like ?";  //?하나만있음
		try {
			pstmt=conn.prepareStatement(sql);
			//바인딩
			pstmt.setString(1, "%"+name+"%");  //이름중 어디든 포함되는 경우
			rs=pstmt.executeQuery();
			System.out.println("시퀀스\t종류\t상품명 \t\t 수량\t단가\t\t입고날짜");
			while(rs.next()) {
				System.out.println(rs.getInt("num")+"\t"+rs.getString("type")+"\t"+rs.getString("sangpum")+" \t \t "+rs.getInt("su")+"\t"+rs.getInt("dan")+"\t"+rs.getString("ipgoday"));
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			System.out.println("1. 상품추가  2. 상품삭제  3. 상품수정  4. 재고출력  5. 상품 검색  9. 종료");
