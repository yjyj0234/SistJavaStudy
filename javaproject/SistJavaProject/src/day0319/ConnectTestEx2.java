...
public class ConnectTestEx2 {
	...
	public void connectBoardAnswer() {
		...
		String sql="select b.bno,writer,subject,nickname,content,writerday "
		...
		try {
			...
			while(rs.next()) {
				...
				System.out.println(bno+"\t"+writer+"\t"+subject+"\t"+nickname+"\t"+content+"\t"+writerday);
			}
	...
	//sawon 부서별 평균급여 최고급여 최저급여
			public void connectSawon() {
				Connection conn=null;
				Statement stmt=null;
				ResultSet rs=null;
				
				String sql="select buseo, to_char(avg(pay),'l999,999,999') avgpay, to_char(max(pay),'l999,999,999') maxpay,min(pay)"
				...
				try {
					...
					while(rs.next()) {
						...
						String avgPay=rs.getString("avgpay");  //to_char로 바꿨으므로 int에서 String 으로 바꿔줌
						String maxPay=rs.getString("maxpay");
						int minPay=rs.getInt("min(pay)");
