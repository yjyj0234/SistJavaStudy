...
class Emp{
	//멤버
	//변수: 사원명(name),기본급여(pay),가족수(famSu), 초과근무시간(timeSu)
	//메서드: setters&getters
	private String name;
	private int pay;
	private int famSu;
	...
	//가족수당, 초과근무수당, 총급여 getFamSudang, getTimeSudang, getTotalPay
	...
	//초과근무수당.. 시간당 근무수당=초과시간당 3만원(단 10시간 이상은 무조건 30만원)
...
public class EmpSalary {
	...
	//emp클래스 결과 출력
	public static void showEmpSalary(Emp[] emp) 
	{
		//제목까지 출력
		Emp.showTitle();
		for(Emp e:emp)
			...
			System.out.println(e.getName()+"\t"+e.getPay()+"\t"+e.getFamSu()+"\t"+e.getTimeSu()+
					"\t"+e.getFamSudang()+"\t"+e.getTimeSudang()+"\t\t"+e.getTotalPay());
	...
	public static void main(String[] args) {
		...
		int inwon;
		Emp [] emp;    //인원 입력이 아직 안되서 new로 할수 없음
		
		System.out.println("입력할 직원수 입력");
